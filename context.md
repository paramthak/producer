...
### Editor UI
- **Live overlay + on-canvas controls** ([SubtitleLayer.tsx](components/editor/SubtitleLayer.tsx)): split into `SubtitleOverlay` (drawn inside the video frame) and `SubtitleToolbar` (rendered by [Preview.tsx](components/editor/Preview.tsx) **below** the frame, in normal flow — NOT overlaid inside it, because the frame's `overflow-hidden` clipped the wider toolbar and cut off the toggle/colour controls). Clicking the caption selects it (`subSelected` in Preview) and reveals the toolbar; dragging moves it vertically (`positionY`; horizontal anchor stays per preset). A **Normal / Highlight target toggle** selects which text the font dropdown · size stepper · colour swatch act on, so base and emphasized text are styled **independently** (`fontFamily`/`fontSize`/`color` vs `highlightFontFamily`/`highlightFontSize`/`highlightColor`).
- **Display cleaning** ([lib/subtitles.ts](lib/subtitles.ts) `cleanCaptionWord`): forced-alignment tokens carry punctuation ("months.", "Hi,", "FD.", "I'm"). For clean word-based captions we strip everything except letters/numbers and *internal* apostrophes/hyphens (contractions like "I'm", "well-known" survive; leading/trailing punctuation and `. , ! ? " ( )` go). Applied at **display time** in `buildCaptionSvg` (overlay + green-screen + burned exports) and the script box — the stored alignment text is untouched so timing/re-chunking are unaffected. Tokens that clean to empty are skipped.
