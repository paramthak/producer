...
export async function matchAndTrim(input: MatchInput, signal?: AbortSignal): Promise<EditPlan> {
  ...
  const sections = windows.map((w) => {
    ...
    return {
      ...
      lines: w.lines.map((l) => ({ id: l.id, text: l.text })),
      candidates,
    };
  });
  ...
  const prompt = [
    ...
    "6. Use as many clips per section as fit naturally — do not pad with one clip alone if multiple candidates are available and the section is long.",
    "7. Never reuse a clipId across two segments unless absolutely necessary.",
    "8. Respect the override prompt if provided. It overrides default choices.",
    "9. Return segments in order from t=0 onward, no overlaps, no gaps.",
    "10. Provide a one-sentence whyClip (why this clip fits this script line) and whyTrim (why this in/out point).",
    "",
